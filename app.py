...
from api_calls import get_competitor_ads, generate_creative_ideas, create_final_ad_text, create_final_ad_image
...
company_description = st.text_input("About Your Company", placeholder="Describe your company...")

# Step 2: Product Details - Collect product name
if company_name and company_description:
    st.header("Product Information")
    product_name = st.text_input("Product Name", placeholder="Enter the product name, you wish to create creative/advertisement for")
    ...
    if product_name:
        ...
        if st.session_state.competitor_ads is None:
            with st.spinner("Fetching competitor ads..."):
                try:
        ...
        if st.session_state.competitor_ads:
            competitor_names = [ad["page_name"] for ad in st.session_state.competitor_ads]
            ...
            if st.session_state.creative_ideas is None:
                with st.spinner("Generating creative ad ideas..."):
                    try:
                        creative_ideas_json = generate_creative_ideas(st.session_state.competitor_ads, product_name)
                        st.session_state.creative_ideas = creative_ideas_json["ad_ideas"]
                    except Exception as e:
                ...
                for i, idea in enumerate(st.session_state.creative_ideas, start=1):
                    ...
                    with col1:
                        if st.button(f"Select Idea {i}", key=f"select_idea_{i}", 
                                   on_click=set_chosen_ad, args=(idea,)):
                            pass
                    with col2:
                        st.write(f"**Ad Text:** {idea['ad_text']}")
                        st.image(idea['image_url'], caption="Idea Image")
                        st.write(f"**Text Idea:** {idea['text_prompt']}")
                        st.write(f"**Image Idea:** {idea['image_prompt']}")
                ...
                if st.session_state.chosen_ad:
                    ...
                    if st.session_state.generate_final and st.session_state.custom_text:
                        ...
                        if st.session_state.final_image and st.session_state.final_text:
                            ...
                            st.write(f"**Final Ad Text:** {st.session_state.final_text['marketing_text']}")
                            st.image(st.session_state.final_image['image_url'], caption="Final Ad Image")
